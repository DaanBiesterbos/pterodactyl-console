...
#[AsCommand(
    name: 'server:start',
    description: 'Send start signal to a server.',
)]
class ServerStartCommand extends AbstractServerPowerCommand
{
    /**
    ...
    protected function execute(InputInterface $input, OutputInterface $output): int
    {
        ...
        try {
            ...
            if (empty($serverId)) {
                ...
                return $this->executeStart($io, $serverId, (bool) $input->getOption('ignore-install'));
            }
     ...
     */
    private function executeStart(SymfonyStyle $io, string $serverId, bool $installIgnored = false): int
    {
        if (false === $installIgnored) {
            $this->awaitInstall($io, $serverId);
        }

        if ($this->pterodactyl->startServer($serverId)) {
            $io->success("Start signal was sent successfully!");
