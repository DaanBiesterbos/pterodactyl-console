...
#[AsCommand(
    name: 'server:stop',
    description: 'Send stop signal to a server.',
)]
class ServerStopCommand extends AbstractServerPowerCommand
{
    /**
    ...
    protected function execute(InputInterface $input, OutputInterface $output): int
    {
        ...
        try {
            ...
            if (empty($serverId)) {
                ...
                return $this->executeStop($io, $serverId, (bool) $input->getOption('ignore-install'));
            }
     ...
     */
    private function executeStop(SymfonyStyle $io, string $serverId, bool $installIgnored = false): int
    {
        if (false === $installIgnored) {
            $this->awaitInstall($io, $serverId);
        }

        if ($this->pterodactyl->stopServer($serverId)) {
            $io->success("Stop signal was sent successfully!");

            return Command::SUCCESS;
        }

        $io->error("Failed to send stop signal...");
