...
#[AsCommand(
    name: 'server:kill',
    description: 'Send kill signal to a server.',
)]
class ServerKillCommand extends AbstractServerPowerCommand
{
    ...
    protected function execute(InputInterface $input, OutputInterface $output): int
    {
        ...
        try {
            ...
            if (empty($serverId)) {
                ...
                return $this->executeKill($io, $serverId, (bool) $input->getOption('ignore-install'));
            }
     ...
     */
    private function executeKill(SymfonyStyle $io, string $serverId, bool $installIgnored = false): int
    {
        if (false === $installIgnored) {
            $this->awaitInstall($io, $serverId);
        }

        if ($this->pterodactyl->killServer($serverId)) {
            $io->success("Kill signal was sent successfully!");

            return Command::SUCCESS;
        }

        $io->error("Failed to send kill signal...");
